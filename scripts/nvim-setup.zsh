...
setup_zsh_config() {
    ...
    if ! grep -q "source.*fzf.*completion" ~/.zshrc; then
        ...
        echo "source $fzf_path/key-bindings.zsh" >> ~/.zshrc
        echo "source $fzf_path/completion.zsh" >> ~/.zshrc
    fi
}
...
if [[ "$(uname)" == "Darwin" ]]; then
    ...
    brew install go node lua luarocks fzf cmake gcc

    # Setup fzf for zsh
    setup_zsh_config ~/.fzf
    ...
    sudo apt install -y golang-go nodejs lua5.4 luarocks fzf cmake
